...
@Injectable()
export class PaymentsCacheService implements OnModuleDestroy {
  private readonly logger = new Logger(PaymentsCacheService.name);
  ...
  private async getRedisClient(): Promise<Redis | null> {
    ...
    this.redisConnectPromise = client
      ...
      .catch((error: unknown) => {
        this.logger.warn(
          `Payments cache will use in-memory fallback because Redis is unavailable: ${
